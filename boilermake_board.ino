...
const byte UNINFECTED=4;
const byte INFECTION=5;
// Global Variables
...
word timer;
int infection;
uint16_t players[2]={0x01AA,0x0253};
...
void loop() {
  ...
  if(timer==0&&infection==1){
    spreadInfection();
    infection=-1;
    
    }
    if(infection==-1&&(minute()*60+second()-startTime>=1)){
      infection=0;
      ledDisplay(3);
    }
   if(minute()*60+second()-startTime>=1){
     startTime=minute()*60+second();
      timer=timer>>1; 
   }
...
void acquireInfection() {
  int i;
  timer=0xFFFF;
   for(i=0;i<sizeof(players)/sizeof(players[0]);i++){
       if(players[i]!=this_node_address){
       struct payload myPayload={UNINFECTED,'\0',{'\0'}};
       size_t len=sizeof(PING) + sizeof('\0')*2;
       radio.openWritingPipe(players[i]);
       radio.stopListening();
...
void spreadInfection() {
  ...
  for(i=0;i<sizeof(players)/sizeof(players[0]);i++){
       if(players[i]!=this_node_address){
       struct payload myPayload={INFECTION,'\0',{'\0'}};
       size_t len=sizeof(PING) + sizeof('\0')*2;
       radio.openWritingPipe(players[i]);
       radio.stopListening();
       radio.write(&myPayload,len);
